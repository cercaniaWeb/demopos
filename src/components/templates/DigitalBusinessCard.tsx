    ...
    return (
        <div className="relative min-h-[100dvh] bg-slate-950 flex flex-col items-center justify-start sm:justify-center p-4 sm:p-8 overflow-x-hidden antialiased selection:bg-cyan-500/30 selection:text-cyan-200">
            ...
            <div className="z-10 w-full max-w-lg flex flex-col gap-6 sm:gap-10 pb-24 sm:pb-0">
                ...
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center justify-between gap-0.5 sm:gap-3 p-1.5 sm:p-2 bg-slate-900/95 backdrop-blur-3xl border border-white/10 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-50 animate-fade-in-up transition-all w-max max-w-[95%] sm:w-auto">
                    <button
                        onClick={handleSaveContact}
                        className="flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-full text-[9px] sm:text-sm font-black uppercase tracking-widest transition-all active:scale-95 whitespace-nowrap shadow-lg shadow-cyan-500/20"
                    >
                        <Download className="w-3 h-3 sm:w-4 sm:h-4 animate-bounce" />
                        <span>Guardar</span>
                    </button>

                    <div className="w-px h-6 bg-white/10 mx-0.5 sm:mx-2"></div>

                    <div className="flex items-center gap-0 sm:gap-2">
                        <a href="https://wa.me/525660951415" target="_blank" rel="noopener noreferrer" className="p-1.5 sm:p-3 text-slate-400 hover:text-green-400 hover:bg-green-500/10 rounded-full transition-all active:scale-95" title="WhatsApp">
                            <MessageCircle className="w-[18px] h-[18px] sm:w-5 sm:h-5" strokeWidth={2} />
                        </a>

                        <a href="mailto:cercaniaweb@gmail.com" className="p-1.5 sm:p-3 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-all active:scale-95" title="Email">
                            <Mail className="w-[18px] h-[18px] sm:w-5 sm:h-5" strokeWidth={2} />
                        </a>

                        <button onClick={handleCopyLink} className="p-1.5 sm:p-3 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-all active:scale-95" title="Copiar Enlace">
                            {copied ? <Check className="w-[18px] h-[18px] sm:w-5 sm:h-5 text-green-400" /> : <Copy className="w-[18px] h-[18px] sm:w-5 sm:h-5" strokeWidth={2} />}
                        </button>

                        <button onClick={handleShare} className="p-1.5 sm:p-3 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-all active:scale-95" title="Compartir">
                            <Share2 className="w-[18px] h-[18px] sm:w-5 sm:h-5" strokeWidth={2} />
                        </button>
                    </div>
                </div>
